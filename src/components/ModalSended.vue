<template>
    <div @click="$emit('close')" class="req" :class="{'req--hide':!isOpen}">
        <div ref="wrap" :style="`transform: scale(${transformWrap})`" @click.stop class="req__wrap" :class="{'req__wrap--hide':!isOpen}">
            ...
            <h2 class="req__wrap_title">{{error ?  'Ошибка' : 'Ваша заявка успешно отправлена'}}</h2>
            <p class="req__wrap_text"> {{error ? 'Во время отправки заявки произошла ошибка' : 'Заявка успешно отправлена. Мы свяжемся с вами в ближайшее время'}}</p>
